...
export class OnchainDataService implements OnModuleInit {
  ...
  private readonly logger: Logger = new Logger(OnchainDataService.name, {
    timestamp: true,
  });
  ...
  private get Connex(): Connex {
    if (this.mConnex === undefined) {
      throw new Error("Connex not initialised");
    }
  ...
  private get FactoryContract(): Connex.Thor.Account.Visitor {
    if (this.mFactoryContract === undefined) {
      throw new Error("FactoryContract not initialised");
    }
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  ...
  private async fetch(): Promise<void> {
    ...
    let method: Connex.Thor.Account.Method =
      this.FactoryContract.method(allPairsLengthABI);
    ...
    const getReservesABI: object = find(VexchangeV2PairABI, {
      name: "getReserves",
    });
    ...
    const promises: Promise<void>[] = times(numPairs, async (i: number) => {
      const res: Connex.VM.Output & Connex.Thor.Account.WithDecoded =
        await method.call(i);
      const pairAddress: string = ethers.utils.getAddress(res.decoded[0]);
      const pairContract: Connex.Thor.Account.Visitor =
        this.Connex.thor.account(pairAddress);

      const token0Address: string = ethers.utils.getAddress(
        (await pairContract.method(token0ABI).call()).decoded[0],
      );

      const token1Address: string = ethers.utils.getAddress(
        (await pairContract.method(token1ABI).call()).decoded[0],
      );
      ...
      const price: BigNumber =
        reserve0BN.eq(0) || reserve1BN.eq(0)
      ...
      const swapFilter: Connex.Thor.Filter<
        "event",
      ...
      this.pairs[pairAddress] = {
        ...
        token0Volume: formatEther(
          parseUnits(accToken0Volume.toString(), 18 - token0.decimals),
        ),
        token1Volume: formatEther(
          parseUnits(accToken1Volume.toString(), 18 - token1.decimals),
        ),
      };
    });
  ...
  private calculateUsdPrices(): void {
    this.tokens[WVET["1"].address].usdPrice =
      this.coingeckoService.getVetPrice();
  ...
  public async onModuleInit(): Promise<void> {
    ...
    await this.getTradingVolumePerPair(
      "0x717829915367308FF113394eB84B174993e19b07",
    );
  ...
  public async getTradingVolumePerPair(pairAddress: string) {
    ...
    const accToken0Volume: BigNumber = ethers.constants.Zero;
    const accToken1Volume: BigNumber = ethers.constants.Zero;
    const ranking = {};
    ...
    while (!end) {
      ...
      for (const transaction of result) {
        console.log(
          ...
          ranking[transaction.decoded.sender],
        );
        if (!ranking[transaction.decoded.sender]) {
          ranking[transaction.decoded.sender] = {
            points: ethers.constants.Zero,
          };
        }

        ranking[transaction.decoded.sender].points = ranking[
          transaction.decoded.sender
    ...
    console.log(
      formatEther(ranking["0x6c0a6e1d922e0e63901301573370b932ae20dadb"].points),
    );
